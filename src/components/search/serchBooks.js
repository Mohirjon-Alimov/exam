...
export const SearchBooks = ({setValue})=>{
  ...
  const handleForm = (evt)=>{
    ...
    fetch(`https://book-service-layer.herokuapp.com/book/search?book=${value}`)
    .then(res => res.json())
  ...
  return(
    <>
      <Wrapper className={theme === 'dark' && 'darkWrapper'}>
        <Text className=' text-center'>{Lang[lang].search}</Text>
        <form onSubmit={handleForm}>
          <div className=' d-flex justify-content-center'>
            <Input 
            className={theme === 'dark' ? 'darkInput': 'lightInput'}
            type="search" placeholder={Lang[lang].searchPlaceholder} />
